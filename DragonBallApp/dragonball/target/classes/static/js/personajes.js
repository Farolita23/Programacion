...
async function fetchConReintentos(url, opciones = {}, maxIntentos = 3) {
    ...
    const btnAgregarTransformacion = document.getElementById("btnAgregarTransformacion");
    if (btnAgregarTransformacion) {
        btnAgregarTransformacion.addEventListener("click", agregarTransformacionAPersonaje);
    }
    ...
    .then(data => {
        ...
        // Si es edición y hay transformaciones, manejarlas
        if (esEdicion && transformacionesPersonajeActual.length > 0) {
            return guardarTransformacionesPersonaje(personajeIdFinal);
        }
...
// Nueva función para actualizar la lista visual de transformaciones del personaje
function actualizarListaTransformacionesPersonaje() {
    ...
    if (transformacionesPersonajeActual.length === 0) {
        lista.innerHTML = `<li class="transformaciones-vacio">No hay transformaciones registradas</li>`;
        return;
    }
    
    transformacionesPersonajeActual.forEach((transformacion, index) => {
        const li = document.createElement("li");
        ...
        li.innerHTML = `
            ...
            <button type="button" class="btn-eliminar-transformacion" data-index="${index}" title="Eliminar transformación">
                ✕
            </button>
        `;
        ...
        li.querySelector(".btn-eliminar-transformacion").addEventListener("click", function() {
            const idx = parseInt(this.getAttribute("data-index"));
            eliminarTransformacionDePersonaje(idx);
        });
    });
}

// Nueva función para agregar una transformación al personaje
function agregarTransformacionAPersonaje() {
    const nombre = document.getElementById("inputNombreTransformacion").value.trim();
    const multiplicador = parseFloat(document.getElementById("inputMultiplicadorTransformacion").value);
    const descripcion = document.getElementById("inputDescripcionTransformacion").value.trim();
    
    // Validaciones
    if (!nombre) {
        mostrarMensaje("El nombre de la transformación es obligatorio", "error");
        return;
    }
    
    if (!multiplicador || multiplicador < 1) {
        mostrarMensaje("El multiplicador debe ser un número mayor o igual a 1", "error");
        return;
    }
    
    if (!descripcion) {
        mostrarMensaje("La descripción de la transformación es obligatoria", "error");
        return;
    }
    
    // Verificar que no exista ya una transformación con el mismo nombre
    if (transformacionesPersonajeActual.some(t => t.nombre.toLowerCase() === nombre.toLowerCase())) {
        mostrarMensaje("Ya existe una transformación con ese nombre", "error");
        return;
    }
    ...
    const nuevaTransformacion = {
        ...
        esNueva: true // Flag para identificar transformaciones nuevas
    };
    
    // Agregar a la lista
    transformacionesPersonajeActual.push(nuevaTransformacion);
    
    // Actualizar la lista visual
    actualizarListaTransformacionesPersonaje();
    
    // Limpiar el formulario
    document.getElementById("inputNombreTransformacion").value = "";
    document.getElementById("inputMultiplicadorTransformacion").value = "";
    document.getElementById("inputDescripcionTransformacion").value = "";
    
    mostrarMensaje(`Transformación "${nombre}" agregada`, "success");
}

// Nueva función para eliminar una transformación del personaje
function eliminarTransformacionDePersonaje(index) {
    if (index >= 0 && index < transformacionesPersonajeActual.length) {
        const transformacion = transformacionesPersonajeActual[index];
        
        if (confirm(`¿Estás seguro de que quieres eliminar la transformación "${transformacion.nombre}"?`)) {
            transformacionesPersonajeActual.splice(index, 1);
            actualizarListaTransformacionesPersonaje();
            mostrarMensaje(`Transformación "${transformacion.nombre}" eliminada`, "info");
        }
    }
}

// Nueva función para guardar las transformaciones del personaje
async function guardarTransformacionesPersonaje(personajeId) {
    // Separar transformaciones nuevas de las existentes
    const transformacionesNuevas = transformacionesPersonajeActual.filter(t => t.esNueva);
    const transformacionesExistentes = transformacionesPersonajeActual.filter(t => !t.esNueva);
    
    try {
        // Primero eliminar las transformaciones que ya no están en la lista
        ...
        for (const transOriginal of transformacionesOriginales) {
            const existeEnActuales = transformacionesExistentes.some(t => t.id === transOriginal.id);
            if (!existeEnActuales) {
                // Eliminar esta transformación
        ...
        // Crear las nuevas transformaciones
        for (const transformacionNueva of transformacionesNuevas) {
            const transformacionData = {
                nombre: transformacionNueva.nombre,
                descripcion: transformacionNueva.descripcion,
                multiplicadorPoder: transformacionNueva.multiplicadorPoder,
                personajeId: parseInt(personajeId)
            };
            
            await fetchConReintentos(`${API_URL}/transformaciones`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json"
                },
        ...
        console.log("Transformaciones guardadas correctamente");
    } catch (error) {
        console.error("Error al guardar transformaciones:", error);
        // No lanzamos el error para no interrumpir el flujo principal
        mostrarMensaje("Personaje guardado, pero hubo problemas con algunas transformaciones", "warning");
    }
}