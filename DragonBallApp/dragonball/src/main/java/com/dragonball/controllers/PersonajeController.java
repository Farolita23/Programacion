...
public ResponseEntity<?> eliminarPersonaje(@PathVariable Long id) {
    try {
        ...
        System.err.println("Error al eliminar personaje con ID " + id + ": " + e.getMessage());
        e.printStackTrace();
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
    ...
    @GetMapping("/{id}/sagas")
public ResponseEntity<?> obtenerSagasPorPersonaje(@PathVariable Long id) {
    try {
        List<Saga> sagas = service.obtenerSagasPorPersonaje(id);
        return ResponseEntity.ok(sagas);
    } catch (SQLException e) {
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body("Error al obtener las sagas del personaje: " + e.getMessage());
    }
}
}